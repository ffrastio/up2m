...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Tambah Pengabdian</h1>
        </div>
        <div class="card">
            <div class="card-body">
                {!! Form::open(['url' => 'pengabdian', 'files' => true]) !!}

                <div class="form-group">
                    {!! Form::label('skim_pengabdian', 'Skim Pengabdian', ['class' => 'control-label']) !!}
                    @if ($list_skim)
                        {!! Form::select('skim_pengabdian', $list_skim, null, ['class' => 'form-control', 'placeholder' => 'Pilih Skim']) !!}
                    @else
                ...
                <div class="form-group">
                    {!! Form::label('nama_ketua_pengabdian', 'Nama Ketua Pengabdian', ['class' => 'control-label']) !!}
                    @if ($errors->any())
                        {!! Form::text('nama_ketua_pengabdian', null, $errors->has('nama_ketua_pengabdian') ? ['id' => 'nama_ketua_pengabdian', 'placeholder' => 'Masukkan nama lengkap', 'class' => 'form-control is-invalid'] : ['id' => 'nama_ketua_pengabdian', 'placeholder' => 'Masukkan nama lengkap', 'class' => 'form-control is-valid']) !!}
                    @else
                        {!! Form::text('nama_ketua_pengabdian', null, ['id' => 'nama_ketua_pengabdian', 'placeholder' => 'Masukkan nama lengkap', 'class' => 'form-control']) !!}
                    @endif
                    <div id="authorList" class="float-left shadow animated--grow-in" aria-labelledby="userDropdown">
                    </div>
                    <span class="form-text text-muted"><i><b>Note:</b> Nama lengkap ketua pengabdian</i></span><br>
                    @if ($errors->has('nama_ketua_pengabdian'))
                        <span class="text-danger">{{ $errors->first('nama_ketua_pengabdian') }}</span>
                    @endif
                </div>
    ...
    <script>
        window.onload = function() {
            $(document).ready(function() {
                ...
                $('#nama_ketua_pengabdian').keyup(function() {
                    var query = $(this).val();
                    if (query != '') {
                ...
                $(document).on('click', 'li', function() {
                    $('#nama_ketua_pengabdian').val($(this).text());
                    $('#authorList').fadeOut();
                });
